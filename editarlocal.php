    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                <?php
                  include("config.php");
                  $sql = "SELECT * FROM locais WHERE id=".$_REQUEST["id"];
                  $res = $conn->query($sql) or die($conn->error);
                  $row = $res->fetch_object();
                ?>
                <form action="salvarlocais.php" method="POST">
                    <input type="hidden" name="acao" value="editarlocal">
                    <input type="hidden" name="id" value="<?php print $row->id; ?>">
                    <div class="form-group">
                        <label>Local</label>
                        <input type="text" name="local_nome" class="form-control" value="<?php print $row->local_nome; ?>">
                    </div>
                    <div class="form-group">
